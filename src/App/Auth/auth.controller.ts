...
export class AuthController {
  ...
  @Post('registration')
  async registration(@Request() req ,@Body() userModel:UserInputModel, @Res() response: Response) {
    let isUserExists = await this.userService.getUserByLoginOrEmail(userModel?.login, userModel?.email)
    if (isUserExists){
  ...
  @Post('login')
  async login(@Request() req ,@Body() userModel:{loginOrEmail:string,password:string}, @Res() response: Response) {
    const user = await this.authService.checkCredentials(userModel.loginOrEmail, req.body.password, userModel.loginOrEmail)
