...
export class AuthService {
  ...
  generateTokens(user: UserDocument) {
    let accessToken =  this.jwtService.sign({user: user.id,email:user.accountData.email,login:user.accountData.login},{secret : this.configService.get('SECRET')})
    let refreshToken =  this.jwtService.sign({user: user.id}, {secret : this.configService.get('SECRET'),expiresIn:'70s'})
    return {
      accessToken,
      refreshToken
    }
  ...
  async saveToken(userId: string, refreshToken: string, ip:string) {
    const { deviceId } = this.jwtService.verify(refreshToken, {secret: this.configService.get('SECRET') })
    const tokenData = await this.authRepository.findTokenByUserId(userId)
    if (tokenData) {
      let status = await this.authRepository.updateToken(userId,refreshToken)
